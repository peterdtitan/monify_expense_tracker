...
def tracker(date=0, income=" ", foodDrinks=" ", Transport=" ", lifeEntertainment=" ", miscellaneous=" ", housing=" ", total_expenses=''):
    ...
    if checkIf(current_month):
        decide = 'w'
        check = True
    database.to_csv(f'{current_month}.csv', mode=decide,
                    header=check, index=False)
...
class Monify:
    ...
    def add_expenditure():
        ...
        if response == 1:
            while True:
                try:
                    date = str(
                        input('Enter expenditure date (YYYY-MM-DD) (optional) \n: --> '))
                    year, month, day = map(int, date.split('-'))
                    date = datetime.date(year, month, day)
    ...
    def add_shopping_list():
        ...
        user_response = input(
            str("Would you like to view your shopping list (yes/no)"))
        user_response.lower
        if user_response == "yes":
...
# STATISTICS TO DISPLAY  VISUAL REPRESENTATION OF EXPESNSE


class Statistics(Monify):

    # DISPLAYS STATIISTICS OF EXPENSE IN A PIE CHART SO USER CAN KNOW WHERE THEIR SPENDING GOES TO
    def spending_chart():
        user_input = input(
            "What month would you like to view the statistics for: ")
        user_input.title()
        slices = []
        headers = ["Food & drinks", "Transport",
                   "Life & Entertainment", "Miscellaneous", "Housing"]
    ...
    '''
        try:
            ...
            ax.legend(title="Expense Categories", loc="upper left",
...
# CREATION OF CSV FILE FOR DEBT RECORDS
...
#CREATION OF EMERGENCY FUNDS RECORDS 
...
def menu():
    ...
    if ans == 1:
        ...
        while True:
            try:
                print("\n\t\t1. Save to Emergency Fund\n\t\t2. View amount in emergency fund\n")
                user_shop_resp = int(input("What do you want to do? --> "))
            except ValueError:
                print("\nPlease enter an integer!\n")
                continue
